...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f9fafb; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          ${sponsorType && sponsorType !== 'N/A' ? `<p><strong>Sponsor Type:</strong> ${sponsorType}</p>` : ''}        </div>
    ...
    console.error('Error sending email:', error);    return res.status(500).json({ 
...
function generateParentContent(data: Record<string, unknown>): string {
  ...
  if (data.students && Array.isArray(data.students)) {
    const studentsHtml = data.students.map((student: Record<string, unknown>, index: number) => `
            <div style="margin-bottom: 10px; padding: 10px; background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 4px;">
              ...
              <p style="margin: 2px 0;"><strong>Name:</strong> ${student.name || 'N/A'}</p>
              <p style="margin: 2px 0;"><strong>Age:</strong> ${student.age || 'N/A'}</p>
              <p style="margin: 2px 0;"><strong>Current Class:</strong> ${student.currentClass || 'N/A'}</p>
              ${student.peculiarity ? `<p style="margin: 2px 0;"><strong>Special Notes:</strong> ${student.peculiarity}</p>` : ''}
            </div>`).join('');
        ...
        <div style="padding: 30px 20px;">
          ...
          <div style="background-color: #fef3c7; padding: 25px; border-radius: 8px; margin: 25px 0;">
            ...
            <ol style="margin: 0; padding-left: 20px; color: #374151;">
              ${acknowledgmentData.nextSteps.map((step) => 
